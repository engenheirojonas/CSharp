...
namespace conexao_mysql_01
{
    public partial class Form1 : Form
    {
        ...
        // Execute este método no construtor do Form1 após o InitializeComponent()
        private void InicializarFiltrosAvancados()
        {
            // 1. Carrega os departamentos cadastrados no Banco de Dados
            cmbFiltroDepartamento.DataSource = dao.ObterDepartamentosUnicos();

            // 2. Configura o ComboBox de Meses de forma estática
            cmbFiltroMes.Items.Clear();
            cmbFiltroMes.Items.Add("Todos");
            for (int i = 1; i <= 12; i++)
            {
                cmbFiltroMes.Items.Add(i.ToString("D2")); // Exibe como 01, 02... 12
            }
            cmbFiltroMes.SelectedIndex = 0; // Padrão: "Todos"

            // 3. Configura o Ano padrão para o ano atual do sistema
            numFiltroAno.Value = DateTime.Now.Year;
        }
        ...
        private void btnFiltrar_Click_1(object sender, EventArgs e)
        {
            try
            {
                // Tratamento moderno de strings com expressões do .NET 10
                string? deptoSelecionado = cmbFiltroDepartamento.Text is "Todos" or "" ? null : cmbFiltroDepartamento.Text;

                int? mesSelecionado = cmbFiltroMes.Text is "Todos" or "" ? null : Convert.ToInt32(cmbFiltroMes.Text);

                int anoSelecionado = Convert.ToInt32(numFiltroAno.Value);

                // Executa a busca filtrada e atualiza a grade de dados (DataGridView)
                dgvFuncionarios.DataSource = dao.PesquisarAvancado(deptoSelecionado, mesSelecionado, anoSelecionado);
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message, "Erro ao Filtrar", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }

        // Evento do Botão Limpar Filtros
        private void btnLimparFiltrar_Click(object sender, EventArgs e)
        {
            cmbFiltroDepartamento.SelectedIndex = 0;
            cmbFiltroMes.SelectedIndex = 0;
            numFiltroAno.Value = DateTime.Now.Year;

            // Exibe novamente todos os funcionários sem restrições
            AtualizarGrade();
        }
    }
}
