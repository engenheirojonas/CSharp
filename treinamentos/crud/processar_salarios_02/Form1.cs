    ...
    public partial class Form1 : Form
    {
        ...
        private void btnImprimir_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (string.IsNullOrEmpty(txtID.Text))
                {
                    MessageBox.Show("Por favor, selecione um funcionário para imprimir.", "Aviso", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }
                ...
                // 2. Configura o objeto de impressão nativo
                using (PrintDocument pd = new PrintDocument())
                {
                    pd.DocumentName = $"Recibo_Vencimento_{_dadosParaImprimir.Id}";

                    // Assina o evento responsável por desenhar a página
                    pd.PrintPage += new PrintPageEventHandler(DesenharReciboPage);

                    // 3. Abre a pré-visualização nativa do Windows (Permite imprimir ou salvar em PDF)
            ...
            {
                MessageBox.Show("Erro ao estruturar impressão nativa: " + ex.Message, "Falha System.Drawing", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
